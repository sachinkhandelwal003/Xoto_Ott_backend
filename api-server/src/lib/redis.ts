...
export async function revokeAllUserTokens(userId: string): Promise<void> {
  if (redis) {
    // In Redis, this would require a different approach (e.g., using a set per user,
    // but for now, let's handle it by iterating (for the in-memory store first
    // For production, you'd want to adjust your data model!
    ...
    // For in-memory, we can't track per user, but this is just a fallback anyway!
